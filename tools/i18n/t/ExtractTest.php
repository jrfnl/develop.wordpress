...
class ExtractTest extends PHPUnit_Framework_TestCase {
	...
	function test_with_just_a_string() {
		$expected = new Translation_Entry(
			array(
				'singular'   => 'baba',
				'references' => array( 'baba.php:1' ),
			)
		);
	...
	function test_entry_from_call_simple() {
		$entry = $this->extractor->entry_from_call(
			array(
				'name' => '__',
				'args' => array( 'baba' ),
			), 'baba.php'
	...
	function test_entry_from_call_plural_and_context() {
		...
		$entry                  = $this->extractor->entry_from_call(
			array(
				'name' => '_nx',
				'args' => array( '%s baba', '%s babas', 'noun' ),
			), 'baba.php'
		);
		$this->assertEquals(
			$entry, new Translation_Entry(
	...
	function test_entry_from_call_extracted_comment() {
		$entry = $this->extractor->entry_from_call(
			array(
				'name'    => '__',
				'args'    => array( 'baba' ),
				'comment' => 'translators: give me back my pants!',
			), 'baba.php'
		);
		$this->assertEquals(
			$entry, new Translation_Entry(
				array(
					'singular'           => 'baba',
					'extracted_comments' => "translators: give me back my pants!\n",
				)
			)
		);
	}
	...
	function test_entry_from_call_multiple() {
		...
		$entries                = $this->extractor->entry_from_call(
			array(
				'name' => 'c',
				'args' => array( 'baba', 'dyado', 'dyados' ),
			), 'baba.php'
		);
		$this->assertEquals(
			array(
				new Translation_Entry( array( 'singular' => 'baba' ) ),
	...
	function test_entry_from_call_multiple_first_plural_then_two_strings() {
		...
		$entries                = $this->extractor->entry_from_call(
			array(
				'name' => 'c',
				'args' => array( 'dyado', 'dyados', 'baba', 'foo', 'bar' ),
			), 'baba.php'
		);
		$this->assertEquals(
			array(
				...
				new Translation_Entry( array( 'singular' => 'bar' ) ),
			), $entries
		);
	}
	...
	function test_find_function_calls_one_arg_literal() {
		$this->assertEquals(
			...
			), $this->extractor->find_function_calls( array( '__' ), '<?php __("baba"); ?>' )
		);
	}
	...
	function test_find_function_calls_one_arg_zero() {
		$this->assertEquals(
			...
			), $this->extractor->find_function_calls( array( '__' ), '<?php __("0"); ?>' )
		);
	}
	...
	function test_find_function_calls_one_arg_non_literal() {
		$this->assertEquals(
			...
			), $this->extractor->find_function_calls( array( '__' ), '<?php __("baba" . "dudu"); ?>' )
		);
	}
	...
	function test_find_function_calls_2_args_bad_literal() {
		$this->assertEquals(
			array(
				array(
					'name' => 'f',
					'args' => array( null, 'baba' ),
					'line' => 1,
				),
			), $this->extractor->find_function_calls( array( 'f' ), '<?php f(5, "baba" ); ' )
		);
	}
	...
	function test_find_function_calls_2_args_bad_literal_bad() {
		$this->assertEquals(
			array(
				array(
					'name' => 'f',
					'args' => array( null, 'baba', null ),
					'line' => 1,
				),
			), $this->extractor->find_function_calls( array( 'f' ), '<?php f(5, "baba", 5 ); ' )
		);
	}
	...
	function test_find_function_calls_1_arg_bad_concat() {
		$this->assertEquals(
			...
			), $this->extractor->find_function_calls( array( 'f' ), '<?php f( "baba" . "baba" ); ' )
		);
	}
	...
	function test_find_function_calls_1_arg_bad_function_call() {
		$this->assertEquals(
			...
			), $this->extractor->find_function_calls( array( 'f' ), '<?php f( g( "baba" ) ); ' )
		);
	}
	...
	function test_find_function_calls_2_arg_literal_bad() {
		$this->assertEquals(
			array(
				array(
					'name' => 'f',
					'args' => array( 'baba', null ),
					'line' => 1,
				),
			), $this->extractor->find_function_calls( array( 'f' ), '<?php f( "baba", null ); ' )
		);
	}
	...
	function test_find_function_calls_2_arg_bad_with_parens_literal() {
		$this->assertEquals(
			...
			), $this->extractor->find_function_calls( array( 'f' ), '<?php f( g( "dyado", "chicho", "lelya "), "baba" ); ' )
		);
	}
	...
	function test_find_function_calls_with_multi_line_comment() {
		$this->assertEquals(
			...
			$this->extractor->find_function_calls(
				array( '__' ),
				"<?php
				/*
